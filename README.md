...
supertonic:
  ...
  candidates: 3      # 한 문장당 생성 후보 수 (best-of-N). 1이면 예전처럼 1회만 생성
...
| `supertonic.candidates` | 발음이 뭉개지는 실패 take를 걸러내는 best-of-N 후보 수 |
...
- **`candidates`** — 한 문장을 N번 생성해 ① 음절 정점 수 ② 마지막 음절 정점 이후 길이
  ③ 전체 발화 길이 순으로 가장 또렷한 take를 고릅니다. ②가 있는 이유는, 음절 수만 보고
  즉시 만족하면 "끝이 짧은" take가 그대로 통과하기 때문입니다. 그래서 **최소 2 take는
  뽑은 뒤** 조기 종료합니다. 짧은 문장 기준 요청당 약 1.8초(1회 생성은 0.9초),
  최악의 경우 N배. 속도가 우선이면 `1`로 두면 예전 동작으로 돌아갑니다.

  마지막 음절 길이의 **절대값**은 끝소리에 좌우됩니다 — 파열음 받침으로 끝나는
  `집합`[지팝]은 60ms대가 정상이고 `하함`은 130ms대가 정상입니다. 그래서 이 값은
  같은 문장의 take끼리 비교할 때만 씁니다. best-of-N이 정확히 그 조건이라 끝소리
  차이가 상수로 상쇄됩니다.
